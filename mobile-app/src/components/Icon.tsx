 ...
 *
 * כיווניות — שימו לב, זו הנקודה העדינה כאן:
 * הגליפים מצוירים בקואורדינטות `left`, אבל React Native *אינו* מתייחס
 * ל-`left` כאל קואורדינטה פיזית תחת RTL. כש-`doLeftAndRightSwapInRTL` דלוק
 * (ברירת המחדל, ואי אפשר לכבות אותה באמינות ב-iOS — `RCTI18nUtil` מאפס אותה
 * ל-true בכל עלייה), מנוע הפריסה ממיר `left` → `start` ו-`borderLeftWidth` →
 * ההתחלה, כלומר כל גליף מצויר *במראה* תחת RTL.
 * לכן אנחנו ממרכזים מחדש ידנית: אייקון רגיל מקבל scaleX(-1) שמבטל את היפוך
 * המנוע ומחזיר אותו לצורה שצוירה, ואייקון כיווני דווקא נשאר הפוך — כי זה
 * מה ש-RTL מחייב. הבדיקה נשענת על שני הדגלים כדי להישאר נכונה גם אם
 * ההתנהגות הזאת תשתנה בעתיד.
 */
...
export function Icon({
  ...
  return (
    <View
      style={[
        { width: S, height: S },
        // ראו ההסבר בראש הקובץ: הגליף כבר מגיע הפוך ממנוע הפריסה תחת RTL,
        // ולכן אייקון רגיל צריך היפוך־נגדי ואייקון כיווני לא.
        needsMirror(dir) && MIRROR,
        style,
      ]}
      pointerEvents="none"
...
/** הדגלים נקראים פעם אחת בעלייה — הם קבועים לכל אורך חיי התהליך. */
const { isRTL: IS_RTL, doLeftAndRightSwapInRTL: SWAPS_EDGES } = I18nManager.getConstants();

/**
 * האם להוסיף scaleX(-1) לגליף.
 *
 * | RTL | המנוע ממיר left→start | אייקון רגיל | אייקון כיווני |
 * |-----|----------------------|-------------|---------------|
 * | לא  | —                    | ללא היפוך   | ללא היפוך     |
 * | כן  | כן (ברירת מחדל)      | היפוך־נגדי  | ללא (כבר הפוך)|
 * | כן  | לא                   | ללא היפוך   | היפוך         |
 */
function needsMirror(dir: boolean): boolean {
  if (!IS_RTL) return false;
  return dir ? !SWAPS_EDGES : SWAPS_EDGES;
}
